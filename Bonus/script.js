...
// Creiamo una variabile per la select (difficoltà)
const select = document.querySelector ("select");

console.log (select.value);

let difficoltàScelta = select.value;

select.addEventListener("change", (e) => {

    console.log(e.target.value);
    difficoltàScelta = e.target.value;
})
...
function showContainer() {
    ...
    for (let i=1; i<=difficoltàScelta; i++) {
        ...
        if (difficoltàScelta === "100") {
        } else if (difficoltàScelta === "81") {
            square.style.width = "calc(100% / 8.8) ";
        } else {
            square.style.width = "calc(100% / 6.5) ";
        }
...
while (positionBomb.length < 16) {
    ...
    let numeroCasuale = genRandomNumMinMax(1, difficoltàScelta);
